...
- 参照中の容量超過本画像10件を第2バッチとして600×600px維持・80,000bytes以下へ再圧縮済み（2026-07-29）
- 参照中の容量超過本画像10件を第3バッチとして600×600px維持・80,000bytes以下へ再圧縮済み（2026-07-29）
- 参照中の容量超過本画像16件を最終バッチとして600×600px維持・80,000bytes以下へ再圧縮済み（2026-07-29）
- `_hash_works_036.webp` を同一作品の高解像度元画像から600×600px・quality 82で再生成済み（2026-07-29）
- Works／Selected Worksカードのキーボード操作、モーダルARIA・閉じるボタン・フォーカス復帰を3ページで対応済み（2026-07-29）
...
- Selected Works規格外3件を600×600px本画像へ統一し、Lienel・雨宮天の高解像度元画像から再生成済み（2026-08-24）
- 規格外8件の元画像を保管し、7件を600×600px本画像へ再生成済み（`_hash_works_030.webp`はquality 50で80,000B超のため見送り、2026-08-24）
- Works本番画像292件を確認済み（600×600: 185件、300×300: 102件、その他: 5件、80,000B超・欠損・WebP以外・NFC違反なし）。規格外は高解像度の同一アートワーク入手時のみ置換するレガシー画像として運用（2026-08-24）
- トップページのヒーロー主要要素を初回描画から表示し、入口演出を最大0.48秒へ短縮・reduced-motion時は無効化（2026-08-24）
- Works初期年のサムネイル取得を先頭8件のみeager・以降lazyへ変更し、idle後の本画像35件一括先読みを廃止（2026-08-24）
...
- 君のメインヒロイン「メロきゅん」、ROOKEY♡ROOKEYS「キミだけショッピングモール (サビver.)」をWorksへ追加し、本画像・サムネイル・元画像を追加（2026-08-25）
- StarLight(上海)「流星ラプソディ」「前向きパーリナイ☆」「恋愛自販機」を公式プレビュー公開日でWorksへ追加し、共通の公式ロゴ画像・元画像を追加（2026-08-25）
- Historyの平成琴姫「通りゃんせ～初恋胸キュンキュン～」・SEVEN4「スキスキスキ」を外部掲載に合わせて表記修正（2026-08-25）
- 2019年以前の作品228件をWeb・X・YouTubeで照合。未確認9曲は削除せず、将来のジャケット画像・根拠資料追加候補として今回の整理対象外（詳細は`docs/03_PROGRESS.md`、2026-08-25）
- History行に任意`thumbnail`対応の小型ジャケット枠と、未設定時のCSS製`NO IMAGE`表示を追加（2026-08-25）
- History全225件を公式情報と照合し、96pxジャケット98画像を166作品へ割り当て。未確認59作品はCSS製`NO IMAGE`を維持（2026-08-25）
- HistoryのキャラクターソングからInstrumental 3件を削除（2026-08-25）
- Historyの600px以下で長いアーティスト名・曲名を省略せず自然改行するよう修正（2026-08-25）
- Worksの年リンク移動前に上位年を生成・対象年サムネイル9枚を先読みし、高速スクロール時の空白と位置ずれを改善（2026-08-25）
